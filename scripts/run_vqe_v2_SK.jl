...
for (k, instance_name) in enumerate(instance_names[loop_var+0:loop_var+19])
    ...
    for j in 1:3
        ...
        for i in 1:n_params_step
            ...
            cost, params, probs, params_hist = optimize_parameters_half(SK_problem, initial_params, niter=niter, learning_rate=learning_rate)
    ...
    #folder_name_w = PATH_w * @sprintf("//vqe_mix_v2_SK_data//origin_data//N_%i//p%i//optimized_parameter_data_20//", N, p);
    #folder_name_w = PATH_w * @sprintf("//vqe_mix_v2_SK_data//origin_data//N_%i//p%i//average_parameter_data//", N, p);
    #folder_name_w = PATH_w * @sprintf("//vqe_mix_v2_SK_data//origin_data//N_%i//p%i//final_parameter_data//", N, p);
